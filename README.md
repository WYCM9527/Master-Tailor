...
文案规则见 `.cursor/rules/prompt-copy.mdc`，核心一句话：**每个事实只出现一次**——效果描述只写体验，数值全在【参数】，技术路线在实现提示 / 参考代码。

- 必须有 `## 效果描述`：三到四句，静止的样子 → 动起来怎样 → 我操作时怎样 →（可选）边界；≤ 200 字；**不写数值、不用 `{{key}}` 占位符、不写实现术语**（validate 强制），要指代参数就用与 `label` 同词的角色词（「强调色」「切换时长」）
- `## 实现提示`：给 AI 的技术路线，只在「附加代码」开关关闭时输出（默认关闭；打开后改为附带参考代码）。它是拿不到参考实现时的唯一规格：写清技术路线，并把**所有决定观感但没有参数化的常量**（尺寸、基准时长、缓动、透明度、混色、投影、阈值）列出来；引用参数用 `{{key}}`，不要写参数的默认数值（validate 会告警）。规则全文见 `.cursor/rules/prompt-copy.mdc`
- 可选 `## 技术要求补充`（追加在全局【技术要求】之后的效果专属要求，如轮播的无障碍 / 键盘 / 暂停约定）与 `## 完成后请检查`（2–3 条，只写容易做错的点，缺省用全局默认三条）
- meta.json 的参数 `help` 会跟在【参数】的值后面，凡是光看数字不知其意的参数都要写
...
- 图标一律取自字节开源 [IconPark](https://iconpark.oceanengine.com/official)（Apache-2.0）：outline / strokeWidth 4 / 方头直角，站点侧经 `src/components/Icons.tsx` 统一包装，效果示例内联 SVG；规则见 `.cursor/rules/icons.mdc`
