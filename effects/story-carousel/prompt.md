## 效果描述
一块 9:16 的竖屏「限时动态」（圆角 {{rounded}}）：{{slides}}。顶部一排分段进度条，每张图对应一段：当前段用 {{accent}} 从左到右匀速填满（每张停留 {{perSlide}}），填满自动切到下一张（快速淡化），已看过的段保持全满，循环播放。进度条下面是渐变彩环头像和名字 {{author}}。交互完全照搬 story 的习惯：点画面左 1/3 回上一张、点其余位置去下一张；按住不放画面暂停（进度条也停），松手继续；鼠标悬停时也暂停。
...
## 放在哪
适合活动倒计时页、产品图故事化展示、社交风个人页；竖版窄容器（320-400px 宽）最正宗
