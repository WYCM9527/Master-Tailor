## 效果描述
一个交叉淡化的图片轮播：容器宽高比 {{ratio}}、圆角 {{rounded}}，展示这些图片——{{slides}}。切换时没有任何位移：旧图原地淡出、新图原地淡入（时长 {{duration}}），两张图在半透明中短暂交叠，像呼吸一样柔和，任意两张之间都能无缝循环。每 {{interval}}自动切换（0 秒表示不自动播放），悬停暂停。有标题的图片底部有深色渐变标题条。分页器样式 {{pager}}（强调色 {{accent}}），「显示左右箭头」为 {{arrows}}。

## 技术要求补充
- 支持鼠标拖拽和触摸滑动切换（用 Pointer Events 统一处理，滑动超过约 50px 判定翻页）
- 支持键盘操作：容器可聚焦，方向键切换上一张 / 下一张；聚焦或悬停时暂停自动播放
- 无障碍：容器带 role="region" 和 aria-roledescription="carousel"，每张幻灯片带 aria-label="第 N 张，共 M 张"
- 页面切到后台时暂停自动播放；系统开启「减少动态效果」时不自动播放、切换为瞬间完成

## 完成后请检查
- 切换是纯淡入淡出，没有滑动位移；交叠过程中不露出底色或闪白
- 自动播放间隔与淡化时长符合参数；悬停时自动播放暂停
- 拖拽、键盘左右键、分页器点击都能切换

## 放在哪
适合想要安静优雅氛围的场景：品牌首页大图、摄影作品展示、相册回顾
