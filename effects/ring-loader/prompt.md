...
- 渐变环：`border-radius: 50%`，`background: conic-gradient(from 0deg, transparent 8%, {{color}}) border-box`（8% 透明缺口即彗尾起点），下面垫 `background-color: rgba(255,255,255, {{track}})` 作轨道（透明段处露出浅白圈），`mask: radial-gradient(farthest-side, transparent calc(100% - 6px), #000 calc(100% - 5px))`（加 `-webkit-mask`）挖出 6px 宽的环；基准 1s、linear。
